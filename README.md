...
The corpus was collected from the China Digital Times (CDT) Chinese-language post archive.
...
For each article, the crawler extracts the `title`, `author`, `source`, `publication date`, `topic`, and `URL`. The main article text is extracted from the <article> element. Website metadata and the “相关阅读” (Related Readings) section are excluded.
...
The crawler produces the raw JSON corpus, which is passed to the
preprocessing module. The preprocessing module generates token-only
data for general corpus analysis and POS-tagged data for
annotation-based searches.

The resulting data can then be independently processed by the analysis
modules. Analysis results, including frequency tables, collocations,
search results, KWIC results, and N-gram statistics, are stored in
`data/results/`.

For tokenization, `Jieba` was chosen because it is a lightweight and widely used Python library for Chinese tokenization. It is designed specifically for Chinese text and provides both tokenization and POS tagging through its `jieba.posseg` module. For the convinience
